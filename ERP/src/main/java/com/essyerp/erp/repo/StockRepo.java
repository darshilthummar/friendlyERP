...
//	void updateTag(String value, Long id);
	...
	@Query(value="select qty from stock_tbl where is_delete=0 and Product_ID=?1 ",nativeQuery=true)
	public long findqty(long productId);
	
	
	@Query(value="select sum(qty) from stock_tbl where is_delete=0",nativeQuery=true)
