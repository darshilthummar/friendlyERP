...
<head>
<meta charset="utf-8" />
<title>Friendly | Edit Purchase</title>
				...
				<div class="mr-auto">
					...
					<ul class="m-subheader__breadcrumbs m-nav m-nav--inline">
						...
						<li class="m-nav__item"><a href="" class="m-nav__link"> <span
								class="m-nav__link-text"> Edit Purchase </span>
						</a></li>
					</ul>
				</div>
			</div>
			...
			<div class="row">
				<div class="col-lg-12">
					...
					<div class="m-portlet">
						...
						<!--begin::Form-->
						<form class="m-form m-form--label-align-right" method="post" action="/purchase/addPurchase/save" id="purchase_form">
							<div class="m-portlet__body">
								...
								<input type="hidden" value="${purchase.id}" name="id">
									...
									<div class="col-lg-4">
									<input type="hidden" value="${purchase.invoiceno}" name="invoiceno">
									<input type="hidden" value="${purchase.prefix}" name="prefix"> 
									<input type="hidden" value="" name="delete_product" id="delete_product"> 
									<input type="text" name="invoice_no" id="inviceNo" class="form-control m-input quantity_id" value="${purchase.prefix}-${purchase.invoiceno}" readonly="readonly">
									</div>
									</div>
									<div class="form-group m-form__group row">
										...
										<div class="col-lg-4">
			                    		 ...
			                    		 <c:choose>
			                    				<c:when test="${supplierList.cid==purchase.supplier.cid}">
			                    					<option value="${supplierList.cid}" selected="selected" >${supplierList.cfname}</option>
			                    				</c:when>
			                    				<c:otherwise>
										...
										<div class="col-lg-4">
										<input type="text" class="form-control daterange-single" name="date" id="date_id" value="${purchase.date}" >
									...
									<div class="form-group m-form__group row">
										...
										<div class="col-lg-4">
											<select class="form-control m-select2" name="paymenttype" id="paymenttype_id">
												...
												<option value="Cash Payment" <c:if test="${purchase.paymenttype == 'Cash Payment'}">selected="selected"</c:if>>Cash Payment</option>
												
												<option value="Bank Payment" <c:if test="${purchase.paymenttype == 'Bank Payment'}"> selected="selected"</c:if>>Bank Payment</option>
											<%-- 
										...
										<div class="col-lg-4">
											...
											<div class="m-input-icon m-input-icon--right">
												<textarea name="details" id="details_id" class="form-control m-input"
													placeholder="Enter product detail" rows="2" cols="3">${purchase.details}</textarea>
											</div>
										</div>
									</div>
						...
						<div class="row">
							...
							<p id="errormessageadditem" style="color:red;"></p>
								<table class="table table-bordered table-framed" id="sales_Table">
									...
									<c:forEach items="${purchase.purchaseitemModel}" var="purchaseitemModel">
									<c:set var="srNo" value="${srNo+1}" scope="page"></c:set>
									 	<tr id="${srNo}" name="${purchaseitemModel.id}" style="cursor: pointer;"> 
							                <td class="sno text-center">${srNo}</td>
						            		<td>
						            		<input type="hidden" name="purchaseitemModel[${srNo}].id" value="${purchaseitemModel.id}"> 
												<div class="form-group" style="margin-bottom: 0px;">
													<div class="m-input-icon m-input-icon--right">										
													<select class="form-control m-input product_id" onchange="calculation(${srNo})" name="purchaseitemModel[${srNo}].product.id" id="product_id${srNo}">
														<option value="0">select Product</option>
															<c:forEach items="${productList}" var="productList">
			                    					 		<c:choose>
			                    								<c:when test="${productList.id==purchaseitemModel.product.id}">
											...
											<td>
												<div class="form-group" style="margin-bottom: 0px;">
													<div class="m-input-icon m-input-icon--right">
														<input type="text" class="form-control m-input" name="purchaseitemModel[${srNo}].quantity" id="quantity_id${srNo}" onkeyup="getTotal(${srNo})" class="form-control m-input quantity_id" value="${purchaseitemModel.quantity}">
													</div>
												</div>
											</td>
											<td>
												<div class="form-group" style="margin-bottom: 0px;">
													<div class="m-input-icon m-input-icon--right">
														<input type="text" name="purchaseitemModel[${srNo}].rate" id="rate_id${srNo}" onkeyup="getTotal(${srNo})" class="form-control m-input rate_id" value="${purchaseitemModel.rate}">
													</div>
												</div>
											</td>
											<td>
												<div class="form-group" style="margin-bottom: 0px;">
													<div class="m-input-icon m-input-icon--right">
														<input type="text" name="purchaseitemModel[${srNo}].total" id="total_id${srNo}" class="form-control m-input total_id" value="${purchaseitemModel.total}">
													</div>
												</div>
											</td>
											<td>
									...
									<div class="form-group m-form__group row">
										...
										<div class="col-lg-2">
											<div class="m-input-icon m-input-icon--right">
												<input type="text" name="grandtotal" id="grandtotal_id" class="form-control m-input m-input--solid"	style="background-color: #E8F0FE;" readonly="readonly" value=${purchase.grandtotal}>
											</div>
										</div>
										<div class="col-lg-2">
